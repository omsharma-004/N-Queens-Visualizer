...
function displayBoard(board) {
    ...
    for (let i = 0; i < boardSize; i++) {
        ...
        for (let j = 0; j < boardSize; j++) {
            ...
            if ((i + j) % 2 === 0) {
                cell.style.backgroundColor = '#ffffff'; // Light square color
            } else {
                cell.style.backgroundColor = '#cccccc'; // Dark square color
            }
            if (board[i][j]) {
...
function markCorrectCell(row, col) {
    ...
    cell.style.backgroundColor = '#00ff00'; // Green color for correct position
}
...
function blinkCell(row, col) {
    ...
    for (let i = 0; i < cells.length; i++) {
        ...
        if (cell.classList.contains('queen')) {
            cell.style.backgroundColor = '#ff0000'; // Red color for blinking
        }
    }
    ...
    for (let i = 0; i < cells.length; i++) {
        for (let j = 0; j < cells.length; j++) {
            if (i !== row && (j === col || Math.abs(row - i) === Math.abs(col - j))) {
                ...
                if (cell.classList.contains('queen')) {
                    cell.style.backgroundColor = '#ff0000'; // Red color for diagonal conflicts
                }
            }
        }
    }
    ...
    setTimeout(() => {
        for (let i = 0; i < cells.length; i++) {
            const cell = cells[i].children[col];
            if (cell.classList.contains('queen')) {
                cell.style.backgroundColor = ((row + col) % 2 === 0) ? '#ffffff' : '#cccccc'; // Revert to original background color
            }
        }
        ...
        for (let i = 0; i < cells.length; i++) {
            for (let j = 0; j < cells.length; j++) {
                if (i !== row && (j === col || Math.abs(row - i) === Math.abs(col - j))) {
                    const cell = cells[i].children[j];
                    if (cell.classList.contains('queen')) {
                        cell.style.backgroundColor = ((i + j) % 2 === 0) ? '#ffffff' : '#cccccc'; // Revert to original background color for diagonal conflicts
                    }
                }
            }
        }
